...
本文件保存兼容旧项目和高风险场景的完整委派合同。普通项目首次创建 Agent、返工或验收使用 `SKILL.md` 与短 runtime 的 V4.1 八字段真实 Thread 合同，不读取本文件；旧七字段任务在下一次消息转换。只有多写入者、高风险/生产工作、正式审计、复杂 Persistent Thread，或旧项目已经依赖十九字段合同时才完整读取。
