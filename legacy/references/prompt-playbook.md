...
SCOPE
<建议触碰：目录/文件清单。
 禁区：只列少数真正不能动的点并附原因（通常 ≤3 条），不写"其他一切不许动"。
 越界规则：实现确需触碰范围外文件时直接做，并在 REPORT 的 DECISIONS 申报；
 要动公共接口、数据格式或发布脚本，先停下来说明再继续。>
...
- SCOPE 是护栏不是紧身衣：约束只写有明确理由的，写不出理由就删掉；功能做对优先于范围收窄，宁可让工作对话越界后申报，也不要让它为守范围绕远路把功能做坏。
...
SCOPE
建议触碰：上述三个文件；需要登记新文件时可改 War3ScriptManager.vcxproj。
禁区：注入与脚本模块的对外行为（其他功能依赖它们）。
越界规则：确有必要就做并在 REPORT 的 DECISIONS 申报；动公共接口先停下说明。
