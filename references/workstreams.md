...
Strategy 存在时，建立、启动、分派或集成候选绑定 Workstream 的前置条件是 Gate 精确为 `OPERATING`。新项目的 pre-bootstrap Strategy-only 阶段不创建空 Workstream、Stage A0、Lead 或长期部门；`BOOTSTRAP_AUTHORIZED` 只允许建立五账本并完成 canonical 协调，不是已经 Operating。

`DISCOVERY_ACTIVE / STRATEGIC_CHOICE_REQUIRED` 中只允许有界的 `discovery-read-only` 或真正无关的只读调研，task-level effective write scope 必须为 `[]`；这些是 Discovery assignments，不伪装成已组建的 Workstream。`DECISION_RECORD_REQUIRED / STATE_SYNC_REQUIRED / EXECUTIVE_APPROVAL_REQUIRED` 中只完成当前 canonical 记账、Thread 同步、报告或安全控制，不派新候选业务。
...
6. ACTIVE 在 `AGENTS.md` 预留不冲突的写入范围后派发。
7. 等待期间检查上游变化；baseline 失效就暂停受影响分支。

可安全并行：互相独立的只读研究、写入不同真实路径且不共享生成物的实现、相互独立的检查。

必须串行：同一文件/资产/数据库迁移、共享生成器输出、生产/发布事务、下游依赖未接受上游、无法可靠切分的工作区。
...
2. ACTIVE 收集 artifact、hash/revision、测试、残余风险和 canonical baseline。
3. 检查 PROJECT 目标、最新 DECISIONS、假设、接口、命名、数据格式、代码/文件冲突、产品/UI/实现一致性、测试、遗漏和新增重大风险。
4. 运行跨线/集成测试或可重复检查。
...
Gate 通过后任何输入 artifact、接口契约、有效 DECISION 或 Strategy semantic context 变化，旧 Gate 自动失效，必须重验。Reviewer/Lead 只能报告 ready/PASS，不能直接推进全局阶段。

## 返工与 stale context

验收不合格时优先发回原 Agent；指出缺陷、证据、必改内容、未变范围和复验标准，不要求无意义全部重做。达到合理返工上限后，ACTIVE 决定更换 Agent、增加 Reviewer/Specialist、修改计划或升级 Founder。

Agent 使用旧 PROJECT、DECISIONS、Strategy context、接口或上游 artifact 时，其受影响成果不得进入 Integration Gate。先重新同步 baseline；能局部修复就定向返工，不能解释差异则阻塞。运行中 Pivot 打开 Gate 时，冻结可能被废弃方向的写入与 Integration；只有真正与选择无关的只读工作可继续。
